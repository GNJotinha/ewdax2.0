...
# ✅ mantém só UMA lista e inclui Desenvolvedor
...
def render(_df, _USUARIOS):
    ...
    with left:
        ...
        if viewing_self:
            # ✅ NOVO: Admin consegue alterar o PRÓPRIO departamento
            if st.session_state.get("is_admin"):
                with st.popover("Alterar meu departamento", use_container_width=True):
                    ...
                    if st.button("Salvar departamento", use_container_width=True, key="pf_self_dept_save"):
                        if dept_new == department:
                            st.info("Nada mudou.")
                            st.stop()
                            ...
                            try:
                                with conn.cursor() as cur:
                                    cur.execute(
                                        ...
                                        where id=%s
                                        """,
                                        (dept_new, my_user_id, my_user_id),
                                    )
                                conn.commit()
                            except Exception as e:
                                conn.rollback()
                                st.error(f"Não consegui atualizar departamento: {e}")
                                st.stop()
    ...
    with right:
        ...
        # Painel de ações: aparece pra admin (e mostra do usuário alvo)
        if st.session_state.get("is_admin"):
