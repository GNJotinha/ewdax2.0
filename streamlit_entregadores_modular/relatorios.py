from utils import normalizar, tempoparasegundos, calculartempoonline
...
def gerartexto(nome, periodo, diasesperados, presencas, faltas, tempo_pct,
...
🚗 Corridas:
• 📦 Ofertadas: {ofertadas}
• 👍 Aceitas: {aceitas} ({txaceitas}%)
• 👎 Rejeitadas: {rejeitadas} ({txrejeitadas}%)
...
Ofertadas: {ofertadas}Aceitas: {aceitas} ({tx_aceitas}%)Rejeitadas: {rejeitadas} ({tx_rejeitadas}%)Completas: {completas} ({tx_completas}%)
"""
def geraralertasde_faltas(df):
...
return mensagens
def gerarporpracadataturno(df, nome=None, praca=None, datainicio=None, datafim=None, turno=None, datas_especificas=None):
